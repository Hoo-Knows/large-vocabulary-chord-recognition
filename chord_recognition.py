...
MODEL_PATH = PACKAGE_ROOT / "cache_data"

MODEL_NAMES = ['joint_chord_net_ismir_naive_v1.0_reweight(0.0,10.0)_s%d.best' % i for i in range(5)]


def chord_recognition(audio_path, lab_path=None, chord_dict_name='submission'):
    hmm = XHMMDecoder(template_file=str(PACKAGE_ROOT.joinpath('data/%s_chord_list.txt' % chord_dict_name)))
    entry = DataEntry()
    ...
    for model_name in MODEL_NAMES:
        net = NetworkInterface(ChordNet(None), model_name, load_checkpoint=False, load_path=str(MODEL_PATH))
