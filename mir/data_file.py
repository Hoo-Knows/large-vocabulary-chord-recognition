...
import os
import gc
from . import io, cache
from .io import LoadingPlaceholder
...
class DataEntryProperties():
    ...
    def set(self, item, value):
        ...
        if(isinstance(value,LoadingPlaceholder)):
            # Set a place holder
            if (item not in self.dict):
                self.dict[item] = value
        elif(item in self.dict and not isinstance(self.dict[item],LoadingPlaceholder)):
            # Old value found
    ...
    def get(self,item):
        ...
        if(item in self.dict):
            ...
            obj=self.dict[item]
            if(isinstance(obj,LoadingPlaceholder)):
                obj.fire()
                obj=self.dict[item]
                assert(not isinstance(obj,LoadingPlaceholder))
            return obj
        else:
...
class DataEntry():
    ...
    def save(self, item, filename, create_dir=False):
        if(create_dir):
            from .cache import mkdir_for_file
